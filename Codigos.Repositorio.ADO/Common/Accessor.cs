        ...
        public Respuesta SpExcuteValidacion(out List<ImagenCodigo> ObjectArray, string storedProcedureName, IEnumerable<SqlParameter>? parameters = null)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                if (reader.Read())
                {
                    ...
                    // Verifica si la columna "ImagenCodigo" existe en el resultado
                    if (availableColumns.Contains("ImagenCodigo"))
                    {
                        try
                        {
                            ObjectArray = reader["ImagenCodigo"] != DBNull.Value ? JsonConvert.DeserializeObject<List<ImagenCodigo>>(reader["ImagenCodigo"].ToString()) : new();
                        }
                        catch (Exception ex)
                        {
                            // Maneja la excepción si es necesario
